...
3. 在 `Bangumi ID / Anitabi URL` 中确认作品，或从 `搜索作品` 中选择作品。
4. 检查地点列表；默认全选，也可以用 `全选`、`全不选`、`反选` 调整。
5. 点击 `加入选中 {count}` 把当前选择加入 `临时清单`。
6. 可继续在 `搜索作品` 中选择其它作品并追加地点。
7. 在 `临时清单` 中点击 `下载 CSV` 或 `下载 KML`。

## 界面语言

右上角 `语言` 菜单提供 `中`、`EN`、`日`。流程按钮会随语言切换，例如中文为 `载入作品`、`搜索作品`、`加入选中 {count}`、`临时清单`、`下载 CSV`、`下载 KML`。

## 普通网页模式

可以直接访问部署后的网页，在 `Bangumi ID / Anitabi URL` 中输入 Bangumi ID 或 Anitabi 地图 URL，点击 `载入作品` 后导出。网页模式同样支持 `搜索作品`、多作品 `临时清单` 和 `下载 CSV` / `下载 KML`。
